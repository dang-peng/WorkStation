    ...
    def accuracy(self, test_data):
        ...
        for data, label in test_data:
            ...
            for predict_label_i, l_i in zip(predict_label, label):
                all += 1
                print("predict_label:", np.argmax(predict_label_i), "label:", np.argmax(l_i))
                if np.argmax(predict_label_i) == np.argmax(l_i):
                    result += 1
        print("True result:", result, "all result:", all, "Accuracy:", result * 100 / all)
        return result / all
...
i = 1
for epoch, _ in enumerate(range(10)):
...
test_batch = DatasetLoader(Test_images, Test_labels, batch_size = 32)

...
class DatasetLoader:
    ...
    def create_data_loader(self):
        ...
        for i in range(0, num_samples, self.batch_size):
            ...
            # _images = [np.reshape(x, (784, 1)) for x in batch_images]
            # _labels = [np.reshape(x, (784, 1)) for x in batch_labels]
            yield batch_images, batch_labels


def datalist(X, Y, batch_size):
    images = []
    labels = []
    train_batch = DatasetLoader(X, Y, batch_size)

...
# average_experiment = np.mean(yb_prob_experiment)
# yb_prob_experiment = np.where(yb_prob_experiment > average_experiment, yb_prob_experiment, 0)  # 处理数据，低于平均值的数置为0
yb_prob = softmax(I0 / (Vr * G0))
# average_softmax = np.mean(yb_prob)
# yb_prob = np.where(yb_prob > average_softmax, yb_prob, 0)  # 处理数据，低于平均值的数置为0
# yb_prob = softmax((I0-Iref) * 1e6 / (Vr * G0))
...
# '#F27D52', '#EA9100', '#DAA100', '#DEB400', '#B7C000', '#CAD239', '#AAC32F', '#9EC147', '#75AF00', '#2CA02C'
filename1 = "./SoftMaxHardware_Final.pickle"
with open(filename1, 'wb') as file:
    pickle.dump((yb_prob_experiment_index_list, yb_prob_index_list, yb_prob_experiment_list, yb_prob_list,), file)
colors = ['#F27D52', '#EA9100', '#DAA100', '#DEB400', '#B7C000', '#CAD239', '#AAC32F', '#9EC147', '#75AF00', '#2CA02C']
...
# plt.legend(prop = {'family': 'Times New Roman'}, loc = 'upper center', bbox_to_anchor = (0.55, 1))

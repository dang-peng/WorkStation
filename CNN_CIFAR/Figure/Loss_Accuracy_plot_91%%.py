...
filename = "../TrainDataVGG19/CNN_train_data_03_09_16_43.pickle"
with open(filename, 'rb') as file:
    (Loss, Test_Accuracy) = pickle.load(file)
...
# 绘制loss和epoch关系图
plt.figure(figsize = (8, 6))
plt.plot(epochs, Test_Accuracy, color = '#69B145',
         label = "VGG19", linewidth = 3)
...
os.makedirs(folder_path, exist_ok = True)
current_time = datetime.datetime.now().strftime("%Y_%m%d")
filename = f"CNN_Test_Accuracy_1_{current_time}.svg"
file_path = os.path.join(folder_path, filename)
plt.savefig(file_path, format = 'svg')
# plt.savefig('output.svg', format = 'svg')
plt.show()
